...
export function SettingsModal({
  ...
  const isSaving = isSavingWorkspace || Boolean(columnSettings?.isSaving)

  async function handleCreate(event: FormEvent<HTMLFormElement>) {
    event.preventDefault()
    const name = newColumnName.trim()
    if (!name || !columnSettings) return
    await columnSettings.onCreate(name)
    setNewColumnName('')
  }
  ...
  async function saveEditing(column: KanbanList) {
    if (!editingName.trim() || !columnSettings) return
    await columnSettings.onUpdate(column, editingName.trim(), editingColor)
    setEditingColumnId(null)
  }

  async function handleDragEnd(result: DropResult) {
    if (!columnSettings || !result.destination || result.source.index === result.destination.index) return
    const reordered = [...columnSettings.columns]
    const [movedColumn] = reordered.splice(result.source.index, 1)
    if (!movedColumn) return
    reordered.splice(result.destination.index, 0, movedColumn)
    await columnSettings.onReorder(reordered)
  }
    ...
    <div className="modal-backdrop" role="presentation" onMouseDown={() => !isSaving && onClose()}>
      ...
      >
        <div className="modal-header">
          <div>
            <h2 id="settings-modal-title">{t.kanbanSettings.title}</h2>
        ...
        {columnSettings ? (
          <section className="settings-section" aria-labelledby="column-settings-title">
            ...
            <form className="new-column-form" onSubmit={handleCreate}>
              ...
              />
              <button className="primary-button" type="submit" disabled={columnSettings.isSaving || !newColumnName.trim()}>
                          ...
                          >
                            ...
                            {editingColumnId === column.id ? (
                              <div className="column-row-actions">
                                <button type="button" disabled={columnSettings.isSaving} onClick={() => setEditingColumnId(null)}>{t.common.cancel}</button>
                                <button type="button" disabled={columnSettings.isSaving || !editingName.trim()} onClick={() => void saveEditing(column)}>{t.common.save}</button>
                              </div>
                            ) : (
                              <div className="column-row-actions">
                                <button
                                  ...
                                  type="button"
                                  disabled={columnSettings.isSaving}
                                ...
                                <button
                                  ...
                                  onClick={() => columnSettings.onDelete(column)}
                                >
                                  <svg viewBox="0 0 24 24" aria-hidden="true">
                                    <path d="M3 6h18M8 6V4h8v2M19 6l-1 14H6L5 6M10 11v5M14 11v5" />
                                  </svg>
